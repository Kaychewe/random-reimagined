...
int main(int argc, char *argv[]) {
  ...
  printf("If you have any make errors or warnings then you will receive a "
         "straight 0.\n");
  printf("Please ensure there are sufficent comments in your code\n");
  printf("Do not foget to add your name and date in comments on top of your "
         "mdadm.c\n");
  printf(
      "/******************************************************************/\n");
  ptr = fopen("mdadm.c", "a+");
...
int test_mount_unmount() {
  ...
  if (rc != 1) {
    printf(
        "failed: mount should succeed on an unmounted system but it failed.\n");
    return 0;
  }

  rc = mdadm_mount();
  if (rc == 1) {
    printf("failed: mount should fail on an already mounted system but it "
           "succeeded.\n");
    return 0;
  }
  ...
  if (rc != 1) {
    printf(
        "failed: unmount should succeed on a mounted system but it failed.\n");
    return 0;
  }

  rc = mdadm_unmount();
  if (rc == 1) {
    printf("failed: unmount should fail on an already unmounted system but it "
           "succeeded.\n");
    return 0;
  }
...
int test_read_invalid_parameters() {
  ...
  if (mdadm_read(addr, SIZE, buf1) != -1) {
    printf(
        "failed: read should fail on an out-of-bound linear address but it did "
        "not.\n");
    return 0;
  }
  ...
  addr = 1048570;
  if (mdadm_read(addr, SIZE, buf1) != -1) {
    printf("failed: read should fail if it goes beyond the end of the linear "
           "address space but it did not.\n");
    return 0;
  }

  uint8_t buf2[2048];
  if (mdadm_read(0, sizeof(buf2), buf2) != -1) {
    printf("failed: read should fail on larger than 1024-byte I/O sizes but it "
           "did not.\n");
    return 0;
  }

  if (mdadm_read(0, SIZE, NULL) != -1) {
    printf("failed: read should fail when passed a NULL pointer and non-zero "
  ...
  if (mdadm_read(0, 0, NULL) != 0) {
    printf(
        "failed: 0-length read should succeed with a NULL pointer but it did "
        "not.\n");
    return 0;
  }
...
int test_read_three_blocks() {
  ...
  uint8_t expected[TEST3_SIZE] = {
      0xaa, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb,
